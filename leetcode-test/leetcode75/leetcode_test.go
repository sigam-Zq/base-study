...
func rankTeamsFix(votes []string) string {
	if len(votes) == 1 {
		return votes[0]
	}
	n := len(votes[0])
	// map[A][i] = j 字符 A 第 i个排名有 j 个
	lMap := make(map[byte]map[int]int)
	for i := 0; i < n; i++ {

		for _, v := range votes {
			if _, ok := lMap[v[i]]; !ok {
				lMap[v[i]] = make(map[int]int, 0)
			}
			if _, ok := lMap[v[i]][i]; !ok {
				lMap[v[i]][i] = 0
			}
			lMap[v[i]][i]++
		}
	...
	sort.Slice(ans, func(i, j int) bool {
		...
		for k := 0; k < n; k++ {
			if lMap[ans[i]][k] != lMap[ans[j]][k] {
				return lMap[ans[i]][k] > lMap[ans[j]][k]
			}
		}
		return ans[i] < ans[j]
	})
...
func rankTeams(votes []string) string {
	...
	for k, v := range lMap {
		// log.Printf(" %c %d \n ", k, v)
		pairList[i].c, pairList[i].score = k, v
		i++
	}

	sort.Slice(pairList, func(i, j int) bool {
		if pairList[i].score == pairList[j].score {
			return pairList[i].c < pairList[j].c
		}
		return pairList[i].score > pairList[j].score
	})
