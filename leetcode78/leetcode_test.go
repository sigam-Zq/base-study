...
func (this *MyCalendar) Book(startTime int, endTime int) bool {
	...
	for _, v := range this.List {
		// 有交集
		// fmt.Printf("i:%d  v.startTime:%d, v.endTime:%d, startTime:%d, endTime:%d\n", i, v.startTime, v.endTime, startTime, endTime)
		// fmt.Printf("v.startTime <= endTime && v.endTime > endTime :%v, v.startTime <= startTime && v.endTime > startTime:%v\n", v.startTime <= endTime && v.endTime > endTime, v.startTime <= startTime && v.endTime > startTime)
		// if !(v.startTime >= startTime || v.endTime < endTime) {
		// 	isPass = false
		// 	break
		// }

		if v.startTime < endTime && v.endTime >= endTime {
			isPass = true
			break
		} else if v.startTime <= startTime && v.endTime > startTime {
			isPass = true
			break
		} else if v.startTime >= startTime && v.endTime < endTime {
			isPass = true
			break
		}
